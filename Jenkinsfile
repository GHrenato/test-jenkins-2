...
pipeline {
 ...
 stages {
        stage('Checkout-git'){
               steps{
		git poll: true, url: 'git@github.com:GHrenato/test-jenkins-2.git'
               }
        }
    ...
    stage('PushDockerImage') {
            steps {
            	sh '''
            		docker tag apptest:latest dockerrenato/apptest:latest
					docker push dockerrenato/apptest:latest
